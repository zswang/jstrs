(function (root, factory) {
    ...
    function base64URIDecode(data) {
        ...
        return decodeUTF8(atob(String(data).replace(/[\-_]/g, function (all) {
            return dict[all];
        })));
    } /*</function>*/
    ...
    function camelCase(text) {
        return String(text)
            .replace(/([a-z][^A-Z]*)([A-Z])|([A-Z])([A-Z][a-z])/g, function () {
            var params = [];
    ...
    function decodeHTML(html) {
        return html.replace(/&((quot|lt|gt|amp|nbsp)|#x([a-f\d]+)|#(\d+));/gi, function () {
            var params = [];
            for (var _i = 0; _i < arguments.length; _i++) {
                params[_i] = arguments[_i];
            }
